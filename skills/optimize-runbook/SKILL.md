...
3. Read the chosen runbook with the Read tool. Extract from frontmatter:
   ...
   - `agent`, `model`, `model_provider`, `snapshot` (if present)
...
- If frontmatter fields are missing (`agent`, `model`, `model_provider`, `snapshot`), propose adding them. Default to `agent: claude-code`, `model: anthropic/claude-sonnet-4.6`, `model_provider: openrouter` unless the trajectories show the runbook is already pinned to a different agent or routing.
